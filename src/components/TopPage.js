...
class TopPage extends Component {
    ...
    loadBlockChainData = async () => {
        ...
        maxRec1 = 2 * this.state.myTotalInvestment - this.state.payoutSum;
        ...
        for (var d = 1; d <= this.state.totalDepositCount; d++) {
            ...
            if (this.state.depAddress === this.state.account) {
                ...
                if (cumRec > this.state.payoutSum) {
                    ...
                    noOfMins = Math.floor(noOfSecs / 10); // 60
                    noOfHours = Math.floor(noOfMins / 60);

                    //  console.log(noOfMins)


                    roi1 = noOfMins * depAmount * this.state.hourRate / this.state.interestRateDivisor;
                    if (roi1 >= depMaxRec) {
                        roi1 = depMaxRec;
                    }
                } else {
